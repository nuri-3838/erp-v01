...
- **Giriş + kullanıcı bazlı ekran yetkisi (v0.1'e BİLİNÇLİ eklendi; normalde v0.7):** Sistem ekipçe kullanılacağı için giriş/şifre + modül→ekran→kullanıcı yetki mimarisi v0.1 kapsamına alındı. Mimari: MUHASEBE modülü → 7 ekran → kullanıcı bazlı erişim (görür/göremez; "değiştiremez" ileride). (Karar: 2026-05-30)
- **Kur farkı / kambiyo kâr-zararı (646/656):** Döviz evrak farklı kur günlerinde açılıp kapandığında (örn. verilen çek @30 kesilir, @36 ödenir) çek/senet hesabında evrak terminal olsa bile TL artığı kalıyor; kambiyo satırı üretilmiyor. Çek-senet'in tüm durum geçişlerinde ve faturalarda aynı desen. Çözüm (ileride): gerçekleşmede giriş kuru üzerinden kapatma + 646/656 fark satırı; CekHesapAyari'na kambiyo hesap alanları. v0.1'de kapsam dışı — "ay sonu manuel yansıtma" kararıyla tutarlı. (Adversaryel inceleme bulgusu, 2026-07-12)

## Yol haritası notu (2026-09-19) — İnsan Kaynakları modülü
- **BİLİNÇLİ kapsam genişletmesi (kullanıcının açık isteği):** menüde "İnsan Kaynakları" modülü — Personel Kartları, İzin Takibi, Özlük Belgeleri (son kullanma tarihli evrak + fotoğraf), Devam/Yoklama; "Yemek Takibi" ekranı da bu modüle taşındı. Yalnız **kayıt tutma**: **bordro, maaş, SGK/vergi/prim, puantaja bağlı ücret hesabı ve muhasebe fişi (yevmiye) BİLİNÇLİ olarak yoktur** — Yemek Takibi gibi kontrol amaçlı operasyonel veri.
- Kişisel veri (TC no, sağlık raporu vb.) hassastır: ekranlar yalnız yetki verilen kullanıcılara açılır; evrak/fotoğraf dosyaları `MEDIA_ROOT` DIŞINDA özel dizinde (`IK_OZEL_DIR`) tutulur ve yalnız yetkili indirme görünümüyle sunulur (nginx `/media/` kimlik doğrulamasızdır).
