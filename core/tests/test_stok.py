...
TR büyük harf, ALT-kategori zorunlu, birim/çevirici/KDV doğrulama, DB kısıtları,
view + yetki. (Miktar/hareket bu fazda YOK.)"""
...
from core.models import Birim, EkranYetki, Stok
...
class StokServisTest(TestCase):
    ...
    def test_guncelle_kod_kategori_sabit(self):
        ...
        self.assertEqual((s.ad, s.uretim_birimi_id, s.kdv_orani),
    ...
    def test_db_cevirici_kisit(self):
        ...
        with self.assertRaises(IntegrityError), transaction.atomic():
            Stok.objects.create(kod="150-10-9001", ad="X", kategori=alt,
                                 ...
                                 cevirici=Decimal("0"), kdv_orani=Decimal("0"))
...
class StokViewTest(TestCase):
    ...
    def test_liste_render_kdv_yok(self):
        ...
        self.assertNotContains(r, "KDV")               # listede KDV gösterilmez
    ...
    def test_ekle_post_otomatik_kod(self):
        ...
        r = self.client.post(reverse("core:stok_ekle"), {
            ...
            "cevirici": "3", "kdv_orani": "20"})
        self.assertEqual(r.status_code, 302)
        s = Stok.objects.get(ad="ALÜMİNYUM LEVHA")
        self.assertEqual((s.kod, s.kategori_id), ("150-10-0001", self.alt.pk))
    ...
    def test_duzenle_post(self):
        s = stok_olustur(ad="levha", kategori_id=self.alt.pk,
                         uretim_birimi_id=self.adet.pk, fatura_birimi_id=self.kg.pk,
                         cevirici=Decimal("1"), kdv_orani=Decimal("20"))
        self.client.force_login(self.yetkili)
        r = self.client.post(reverse("core:stok_duzenle", args=[s.pk]), {
            "ad": "levha yeni", "uretim_birimi": str(self.kg.pk),
            "fatura_birimi": str(self.adet.pk), "cevirici": "2,5", "kdv_orani": "10"})
        self.assertEqual(r.status_code, 302)
        s.refresh_from_db()
        self.assertEqual((s.ad, s.kod, s.cevirici), ("LEVHA YENİ", "150-10-0001", Decimal("2.500000")))
